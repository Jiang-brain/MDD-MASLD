######################################PSM#####################################################################################
library(MatchIt)
index=which(datas$depression==0|datas$depression==1)
temp_datas=datas[index,]
rm(index)

row.names(temp_datas)=c(1:nrow(temp_datas))
#exact=c('race','alcohol','sedentary','age'),
m.out1 <- matchit(depression ~ age+gender+race+deprivation+university_education+activity+
                    smoking+alcohol+metabolic_syndrome,caliper = 0.05,
...
library("survival")
library("survminer")
model <- coxph(Surv(time, status) ~ depression,data = new_data)
model1=summary(model)
rm(model)

##############################demographic data for matched data#######################
summary_information=data.frame(matrix(ncol = 4, nrow =30))
colnames(summary_information)=c('Whole population','No depression','Depression','P for difference')
...
for (depression_status in c(10,0,1)) {
  if (depression_status==10){
    index_depression=c(1:nrow(new_data))#10=whole samples
    column_index=1
  }else{
    index_depression=which(new_data$depression==depression_status)
    column_index=depression_status+2
  }
  temp_datas=new_data[index_depression,]
  rm(index_depression)
  #N
...
rm(depression_status,column_index)
#P-value
{
  #age
...
#write.csv(summary_information,'/Users/andyjiang/OneDrive/rtjiang/G/working_data/2024-frailty_Nafld/datas/demographic_depression_masld_psm.csv')
